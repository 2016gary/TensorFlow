...
# os.environ['TF_CPP_MIN_LOG_LEVEL'] = '2'  # 屏蔽没有编译tf源码时的警告（据说编译源码速度能快4-6倍）

# step 1：构造Graph 提取数据的OP
TRAIN_TFRECORD = 'F:/10-image-set2/train.tfrecords'  # 训练数据集
VAL_TFRECORD = 'F:/10-image-set2/val.tfrecords'  # 验证数据集
WIDTH = 100  # 图片的宽
HEIGHT = 100  # 图片的高
CHANNEL = 3  # 图片通道色数
TRAIN_BATCH_SIZE = 64  # 一次取多少张图片进行训练，计算能力有限只能按批次进行训练
VAL_BATCH_SIZE = 16
...
x_train_batch, y_train_batch = tf.train.shuffle_batch([train_img, train_label],  # 使用shuffle_batch随机打乱
                                                      batch_size=TRAIN_BATCH_SIZE, capacity=80000,
                                                      min_after_dequeue=79999, num_threads=16,
...
x_val_batch, y_val_batch = tf.train.shuffle_batch([val_img, val_label],
                                                  ...
                                                  min_after_dequeue=19999, num_threads=16, name='val_shuffle_batch')
...
with tf.Session() as sess:
    ...
    for epoch in range(MAX_EPOCH):
        # training
        train_step = int(80000 / TRAIN_BATCH_SIZE)  # 总共迭代800次
